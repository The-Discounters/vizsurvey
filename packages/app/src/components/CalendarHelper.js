...
export const drawCalendar = ({
    ...
    week.map((day) => {
      ...
      return {
        ...
        amount:
          date === null
            ? null
            : date.toMillis() === question.dateEarlier
            ? question.amountEarlier
            : date.toMillis() === question.dateLater
            ? question.amountLater
            : null,
        type:
          date === null
            ? AmountType.none
  ...
  const iconAmtInc = question.maxAmount / (numIconRow * numIconCol);
  const iconAmtData = range(0, question.maxAmount, iconAmtInc);
    ...
    .join(
      (enter) => {
        ...
        enter
          ...
          .on("click", (d) => {
            if (
              question.interaction === InteractionType.titration ||
              question.interaction === InteractionType.none
            ) {
  ...
  if (question.interaction === InteractionType.drag) {
    var dragHandler = drag().on("drag", function (d) {
      if (d.subject.type === question.variableAmount) {
        const newAmount = yBar.invert(d.y);
        if (newAmount > question.maxAmount) return;
